...
func TestOpen(t *testing.T) {
	d, _ := Open("tests/open.db")
	//fmt.Println(d)
	Set("tests/open.db", []byte("foo"), []byte("bar"))
	//val, ok := d.ReadKey("foo")
	fmt.Println(Get("tests/open.db", []byte("foo")))
	d.deleteKey("foo")
	_, ok := d.readKey("foo")
	fmt.Println(ok)
}
...
func TestSet(t *testing.T) {
	...
	//ch(err, t)
	defer CloseAll()
	val, err := Get("tests/nodb.db", []byte("1"))
	logg(val)

	err = Set("tests/1.db", []byte("1"), []byte("11"))
	ch(err, t)
	err = Set("tests/1.db", []byte("2"), []byte("22"))
	ch(err, t)
}

func TestGet(t *testing.T) {
	defer CloseAll()
	Set("tests/1.db", []byte("1"), []byte("11"))
	res, err := Get("tests/1.db", []byte("1"))
	if err != nil {
		t.Error(err)
	}
	logg("Get:" + string(res))
	res2, err2 := Get("tests/1.db", []byte("2"))
	if err2 != nil {
		t.Error(err2)
	}
	logg("Get:" + string(res2))
	keys, err := Keys("tests/1.db", nil, 0, 0, true)
	ch(err, t)

	result := Gets("tests/1.db", keys)
	_ = result
	//logg(result)
}
